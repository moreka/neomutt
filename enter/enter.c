  ...
  while (true)
  {
    if (!wdata.pass)
    {
      ...
      if ((wdata.state->curpos < wdata.state->begin) ||
          (mutt_mb_wcswidth(wdata.state->wbuf + wdata.state->begin, wdata.state->curpos - wdata.state->begin) >= width))
      {
        wdata.state->begin = mutt_mb_width_ceiling(
            wdata.state->wbuf, wdata.state->lastchar,
      ...
      mutt_window_move(win,
                       wdata.col + mutt_mb_wcswidth(wdata.state->wbuf + wdata.state->begin,
