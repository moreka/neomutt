                           ...
                           int *numfiles, struct EnterState *state)
{
  ...
  else
  {
    ...
    state->wbuflen = 0;
    state->lastchar =
        mutt_mb_mbstowcs(&state->wbuf, &state->wbuflen, 0, mutt_buffer_string(buf));
      ...
      switch (ch)
      {
        ...
        case OP_EDITOR_MAILBOX_CYCLE:
          if (flags & MUTT_COMP_FILE_MBOX)
          {
            ...
            state->curpos = state->lastchar = mutt_mb_mbstowcs(
                &state->wbuf, &state->wbuflen, 0, mutt_buffer_string(buf));
            break;
          }
        ...
        case OP_EDITOR_COMPLETE_QUERY:
          ...
          if (flags & MUTT_COMP_FILE_SIMPLE)
          {
            ...
            {
              mutt_buffer_select_file(buf, MUTT_SEL_NO_FLAGS, m, NULL, NULL);
      ...
      if ((wc == '\r') || (wc == '\n'))
      {
        ...
        if (multiple)
        {
          ...
          *numfiles = 1;
          ...
          tfiles[0] = mutt_buffer_strdup(buf);
          *files = tfiles;
        }
        rc = 0;
        goto bye;
